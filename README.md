...
- Supports browsers: IE11+, Chrome, Firefox, Safari

## Install

[![rc-select](https://nodei.co/npm/selectable-options.png?mini=true)](https://npmjs.org/package/selectable-options)
...
| maxTagPlaceholder | Placeholder shown for truncated tags | ReactNode/function(truncatedTagValues) | - |
...
| onChange | called when select an option or input value change(combobox) | (value, option: Option \| Option[]) => void | - |
...
| getInputElement | customize input element | () => Element | - |
...
| label | The selectable option's label as it appears when it is selectable in the options dropdown as well as how it appears when shown as a selected option (this can be overridden using props `optionSelectedLabelProp` and `optionSelectableLabelProp`) | string \| React.Element | - |
| selectableLabel* | The option's label as it appears when it is selectable in the options dropdown (*there is nothing special about the "selectableLabel" property*, it only becomes meaningful when the `optionSelectableLabelProp` is set to "selectableLabel") | string \| React.Element | - |
| selectedLabel* | The option's label as it appears when shown as a selected option (*there is nothing special about the "selectedLabel" property*, it only becomes meaningful when the `optionSelectedLabelProp` is set to "selectedLabel") | string \| React.Element | - |
...
### Option Group (Structured Data)

| name | description | type | default |
| --- | --- | --- | --- |
| label | The option group's label as it appears in the selectable options dropdown | string \| React.Element | - |
